...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        case "start_vib3_test": {
            const { testFile = "comprehensive-mcp-vib3-test.html", baseUrl = vib3BaseUrl } = request.params.arguments;
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to start VIB3 test: ${error.message}`);
            }
        }
        case "test_vib3_phase": {
            const { phase } = request.params.arguments;
            if (!page) {
            ...
            try {
                const result = await page.evaluate(async (phaseNum) => {
                    // Test specific phase
                    const results = { phase: phaseNum, tests: [] };
                    switch (phaseNum) {
                        case 1: // Core Architecture
                            if (window.VIB34D_Phase1) {
                                results.tests.push({
                                    name: "HypercubeCore",
                                    available: !!window.VIB34D_Phase1.HypercubeCore
                                });
                            }
                            break;
                        case 2: // Geometry Implementations
                            if (window.VIB34D_Phase2) {
                                const geometries = ['Hypercube', 'Hypersphere', 'Hypertetrahedron', 'Torus', 'KleinBottle', 'Fractal', 'Wave', 'Crystal'];
                                geometries.forEach(geom => {
                                    results.tests.push({
                                        name: `${geom}Geometry`,
                                        available: !!window.VIB34D_Phase2[`${geom}Geometry`]
                                    });
                                });
                            }
                            break;
                        case 3: // Projection System
                            if (window.VIB34D_Phase3) {
                                results.tests.push({
                                    name: "ProjectionManager",
                                    available: !!window.VIB34D_Phase3.ProjectionManager
                                });
                            }
                            break;
                        case 4: // Shader Uniforms
                            if (window.VIB34D_Phase4) {
                                results.tests.push({
                                    name: "EnhancedShaderManager",
                                    available: !!window.VIB34D_Phase4.EnhancedShaderManager
                                });
                            }
                            break;
                        case 5: // Interaction Integration
                            if (window.VIB34D_Phase5) {
                                results.tests.push({
                                    name: "VIB34DInteractionEngine",
                                    available: !!window.VIB34D_Phase5.VIB34DInteractionEngine
                                });
                            }
                            break;
                        case 6: // Chromatic Integration
                            if (window.VIB34D_Phase6) {
                                results.tests.push({
                                    name: "VIB34DChromaticParameterBridge",
                                    available: !!window.VIB34D_Phase6.VIB34DChromaticParameterBridge
                                });
                            }
                            break;
                        case 7: // VIB3 Integration
                            if (window.VIB34D_Phase7) {
                                results.tests.push({
                                    ...
                                    available: !!window.VIB34D_Phase7.VIB34DVIb3IntegrationBridge
                                });
                            }
                            break;
                    ...
                    results.success = results.tests.every((t) => t.available);
                    return results;
                }, phase);
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to test phase ${phase}: ${error.message}`);
            }
        }
        case "get_vib3_metrics": {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to get metrics: ${error.message}`);
            }
        }
        case "control_vib3_parameters": {
            ...
            try {
                const result = await page.evaluate((params) => {
                    ...
                    try {
                        window.vib3System.hypercubeCore.updateParameters(params);
                        return {
                            success: true,
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to control parameters: ${error.message}`);
            }
        }
        case "trigger_vib3_interaction": {
            const { interaction, data = {} } = request.params.arguments;
            if (!page) {
                throw new McpError(ErrorCode.InvalidRequest, "VIB3 test not started");
            }
            try {
                const result = await page.evaluate((type, interactionData) => {
                    switch (type) {
                        case 'click':
                            const clickBtn = document.querySelector('button[onclick="triggerInteraction()"]');
                            if (clickBtn)
                                clickBtn.click();
                            break;
                        case 'scroll':
                        ...
                        case 'phase_transition':
                            const phaseBtn = document.querySelector('button[onclick="testPhaseTransition()"]');
                            if (phaseBtn)
                                phaseBtn.click();
                            break;
                    }
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to trigger interaction: ${error.message}`);
            }
        }
            ...
            try {
                const analysis = await page.evaluate(() => {
                    ...
                    if (window.vib3System) {
                        ...
                        try {
                            results.currentState = window.vib3System.getSystemAnalysis();
                        }
                        catch (e) {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to analyze state: ${error.message}`);
            }
        }
        case "run_vib3_test_suite": {
            const { phases = [1, 2, 3, 4, 5, 6, 7, 8] } = request.params.arguments;
            if (!page) {
                throw new McpError(ErrorCode.InvalidRequest, "VIB3 test not started");
            }
            try {
                ...
                for (const phase of phases) {
                    const phaseResult = await page.evaluate(async (phaseNum) => {
                        ...
                        return { phase: phaseNum, success: !!window[`VIB34D_Phase${phaseNum}`] };
                    }, phase);
                ...
                await page.evaluate(() => {
                    ...
                    const initBtn = document.querySelector('button[onclick="initVIB3()"]');
                    if (initBtn)
                        initBtn.click();
                });
        ...
        case "close_vib3_test": {
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to close test: ${error.message}`);
            }
        }
        default:
